...
class Proposals(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        I, target = self.waste2.__getitem__(img_id)
...
def get_dataloaders_proposals(batch_size, num_workers=8, seed=42, data_path='/dtu/datasets1/02514/data_wastedetection', proposal_path = 'region_proposals2', size=64):
    
    
    dataset = Proposals(data_path=data_path, proposal_path = proposal_path, size=size)
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = get_dataloaders_proposals(batch_size=1, num_workers=0, seed=42, data_path=path, proposal_path = 'region_proposals3', size=64)
    img, target, prop, rect, img_id = next(iter(test_loader))
